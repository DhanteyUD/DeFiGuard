...
fraud_agent_av = Agent(
    name="fraud_detection",
    mailbox=True
)

print(f"Fraud Detection Agent (Solana Enhanced) Address: {fraud_agent_av.address}")
...
@fraud_agent_av.on_message(model=TokenAnalysisRequest)
...
@fraud_agent_av.on_event("startup")
async def startup(ctx: Context):
    ctx.logger.info("=" * 60)
    ctx.logger.info("🕵️  DeFiGuard Fraud Detection (Solana Enhanced)")
    ctx.logger.info(f"📍 Agent Address: {fraud_agent_av.address}")
